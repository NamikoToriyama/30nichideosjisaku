...
void HariMain(void)
{
	...
	timer_settime(timer, 1000); // 10
	fifo8_init(&timerfifo2, 8, timerbuf2);
	timer2 = timer_alloc();
	timer_init(timer2, &timerfifo2, 1);
	timer_settime(timer2, 300); //3
		...
		if (fifo8_status(&keyfifo) + fifo8_status(&mousefifo) + fifo8_status(&timerfifo)
			...
			if (fifo8_status(&keyfifo) != 0) {
				...
				if (mouse_decode(&mdec, i) != 0) {
					...
					putfonts8_asc(buf_back, binfo->scrnx, 0, 0, COL8_FFFFFF, s); /* 座標描く */
					sheet_refresh(sht_back, 0, 0, 80, 16);	/* sheet_refreshを含む */
					sheet_slide(sht_mouse, mx, my);
				}
				...
				io_sti();
				if (i != 0) {
					timer_init(timer3, &timerfifo3, 0); /* カーソルもどき */
					boxfill8(buf_back, binfo->scrnx, COL8_FFFFFF, 8, 96, 15, 111);
				} else {
					timer_init(timer3, &timerfifo3, 1); /* 次は0を */
