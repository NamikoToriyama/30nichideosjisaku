...
void cons_putchar(struct CONSOLE *cons, int chr, char move)
{
	...
	if (s[0] == 0x09) {	/* タブ */
		for (;;) {
			...
			cons->cur_x += 8;
			if (cons->cur_x == 8 + 240) { /* 文字数がいっぱいの時 */
				cons_newline(cons);
			}
...
/* コマンドラインからファイル名を生成 */
	...
	if (finfo != 0) {
		...
		if (finfo->size >= 36 && strncmp(p + 4, "Hari", 4) == 0 && *p == 0x00) {
			// RETF命令で終了できないため6バイトの書き換えがいらなくなる
			// サイズを測る
			segsiz = *((int *) (p + 0x0000));
			esp    = *((int *) (p + 0x000c));
			datsiz = *((int *) (p + 0x0010));
...
int *hrb_api(int edi, int esi, int ebp, int esp, int ebx, int edx, int ecx, int eax)
{
	...
	if (edx == 1) {
		...
		if ((ebx & 1) == 0) { // ebxが偶数の時は自動でリフレッシュを行う
		...
		ecx = (ecx + 0x0f) & 0xfffffff0; /* 16バイト単位に切り上げ */
		memman_free((struct MEMMAN *) (ebx + ds_base), eax, ecx);
	} else if (edx == 11) { // ウィンドウに点をうつ
		sht = (struct SHEET *) (ebx & 0xfffffffe);
		sht->buf[sht->bxsize * edi + esi] = eax;
		if ((ebx & 1) == 0) {
			sheet_refresh(sht, esi, edi, esi + 1, edi + 1);
		}
	} else if (edx == 12) { // sheetの自動リフレッシュ管理
		sht = (struct SHEET *) ebx;
		sheet_refresh(sht, eax, ecx, esi, edi);
	} else if (edx == 13) { // ウィンドウに線を引く
		sht = (struct SHEET *) (ebx & 0xfffffffe);
	...
	} else if (edx == 15) { // キーチェックの機能、Enterを押すとウィンドウが閉じる
		for (;;) {
			io_cli();
			if (fifo32_status(&task->fifo) == 0) {
				if (eax != 0) {
			...
			io_sti();
			if (i <= 1) { /* カーソル用タイマ */
				/* アプリ実行中はカーソルが出ないので、いつもつぐは表示用の1にしておく */
...
void hrb_api_linewin(struct SHEET *sht, int x0, int y0, int x1, int y1, int col)
{
	...
	dx = x1 - x0;
	dy = y1 - y0;
	// 1024で割るために10ビットシフトする
	// 割るのは少数でないと綺麗な直線を描くことができないため
	x = x0 << 10;
	y = y0 << 10;
	if (dx < 0) {
		dx = - dx;
